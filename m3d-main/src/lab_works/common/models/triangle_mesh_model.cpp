...
namespace M3D_ISICG
{
	...
	void TriangleMeshModel::render1( const GLuint p_glProgram ) 
	{
		for ( size_t i = 0; i < _meshes.size(); i++ )
		{
			_meshes[ i ].render1( p_glProgram );
		}
	}
	...
	{
		const std::string meshName = _name + "" + std::string( p_mesh->mName.C_Str() );
		if ( VERBOSE )
